...
[SerializeField]
    ...
    void Update()
    {
        ...
        if(grounded && Input.GetKey(KeyCode.Space)&& Time.time-lastJumpTime>=jumpCoolDown)
        {   
        ...
        controller.Move(velocity);
         ...
         for(int i=0;i<groundReferences.Count;i++)
         {
            ...
            bool hit=Physics.Raycast(new Ray(groundRef.position+Vector3.up,Vector3.down),out raycastHit,1.2f+Mathf.Abs(velocity.x*2.0f),groundMask);
            if(hit)
            {
